...
fn main() {
    ...
    let shader_program = pi3d::shader::Program::from_res(
          &display.res, "shaders/triangle").unwrap();
    shader_program.set_used(); // put in pi3d::buffer.draw()
    let vertices: Vec<f32> = vec![-0.25, -0.25, -0.25, 1.0, 0.0, 0.0, // put in pi3d::buffer
    ...
    unsafe {
        ...
        gl::VertexAttribPointer(shader_program.get_attribute_location("vertex"),
                      3, gl::FLOAT, gl::FALSE, stride, std::ptr::null());
        gl::EnableVertexAttribArray(1);
        gl::VertexAttribPointer(shader_program.get_attribute_location("normal"),
