...
fn main() {
    ...
    // shaders
    let shader = pi3d::shader::Program::from_res("uv_bump").unwrap();
    let shinesh = pi3d::shader::Program::from_res("mat_reflect").unwrap();
    ...
    // environment cube
    let (mut ecube, _tex_list) =
        pi3d::shapes::environment_cube::create(camera.reference(), 900.0, "ecubes/sbox", "jpg");
    ...
    mymap.shape.set_draw_details(
        &shader,
        &vec![mountimg1.id, bumpimg.id, reflimg.id],
        128.0,
        0.0,
        1.0,
        1.0,
    ...
    let mut mytrees2 = pi3d::shapes::merge_shape::create(camera.reference());
    pi3d::shapes::merge_shape::cluster(
        &mut mytrees2,
        &treemodel2,
        &mymap,
    ...
    let mut mytrees3 = pi3d::shapes::merge_shape::create(camera.reference());
    pi3d::shapes::merge_shape::cluster(
        &mut mytrees3,
        &treemodel2,
        &mymap,
    ...
    let (mut monument, _tex_list) =
        pi3d::shapes::model_obj::create(camera.reference(), "models/rust_pi3d.obj");
    monument.set_shader(&shinesh);
    monument.set_normal_shine(
        &vec![bumpimg.id, reflimg.id],
        16.0,
        0.02,
        1.0,
        1.0,
    ...
    while display.loop_running() {
        ...
        if z.abs() > 300.0 {
            ...
            if x.abs() > 300.0 {
                mymap
                    .shape
                    .position(&[mapsize * x.signum(), 0.0, mapsize * z.signum()]);
                mymap.shape.draw();
            }
        }
        ...
        if x.abs() >= halfsize {
            //TODO what causes momentary limbo crossing border?
            x -= x.signum() * mapsize;
        }
        if z.abs() >= halfsize {
            z -= z.signum() * mapsize;
