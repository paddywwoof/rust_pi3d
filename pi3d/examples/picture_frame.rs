...
fn get_files(dir: &Path, file_list: &mut Vec<String>) {
    for entry in fs::read_dir(dir).unwrap() {
        // will panic if dir is not a Path
        match entry {
            Ok(dir_entry) => {
                ...
                if path.is_dir() {
                    ...
                    // it's a file, check if image TODO exif dates and rotation
...
fn main() {
    ...
    let mut display =
        pi3d::display::create("picture_frame ESC to quit", 100.0, 100.0, "GL", 2, 1).unwrap();
    ...
    while display.loop_running() {
        ...
        if delta_tm.as_secs() > time_delay.as_secs() {
            ...
            loop {
                ...
                if (sfg.is_some() && sbg.is_some()) || loop_i > 5 {
                    break;
                }
            }
            ...
            let (ix0, ix1) = if wh_rat > 1.0 { (0, 1) } else { (1, 0) };
