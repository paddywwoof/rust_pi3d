...
dog\"",
    ...
    let mut candlestick = pi3d::shapes::lathe::create(
        camera.reference(),
        vec![
            [0.0, 2.0],
            [0.1, 1.8],
    ...
    candlestick.set_draw_details(
        &shader_program,
        &vec![tex.id, mapnorm.id, stars.id],
        1.0,
        0.1,
        1.0,
        1.0,
    ...
    let sphere =
        pi3d::shapes::sphere::create(camera.reference(), 1.5, 16, 32, 0.3, false).reference();
    ...
    let mut map = pi3d::shapes::elevation_map::new(
        camera.reference(),
    ...
    map.shape.set_draw_details(
        &shader_program,
        &vec![maptex.id, mapnorm.id, stars.id],
        128.0,
        0.0,
        1.0,
        1.0,
    ...
    pi3d::shapes::merge_shape::cluster(
        &mut clust, &cube2, &map, -20.0, -100.0, 200.0, 150.0, 0.5, 2.5, 200,
    );
    ...
    while display.loop_running() {
        ...
        sphere.borrow_mut().position(&[
            (t * 0.087 % 2.2 - 1.1).abs(),
            (t * 0.12 % 5.98 - 2.99).abs() + 4.0,
            2.5,
        ]);
        ...
        if display.keys_down.contains(&Keycode::A) {
            cube2.offset(&[t % 3.0, 0.0, 0.0]);
        }
        if display.mouse_moved {
            tilt = (display.mouse_y as f32 - 300.0) * -0.004;
            rot = (display.mouse_x as f32 - 400.0) * -0.004;
        }
        if display.keys_pressed.contains(&Keycode::L) {
            candlestick.buf[0].set_line_width(2.0, true, false);
        }
        if display.keys_pressed.contains(&Keycode::F) {
            candlestick.buf[0].set_line_width(0.0, true, false);
        }
        if display.keys_pressed.contains(&Keycode::P) {
            candlestick.buf[0].set_point_size(3.0);
        }
        ...
        if display.was_resized() {
            ...
            fps_text.set_position(
                &font,
                fps_blk,
                &[
                    -display.width * 0.5 + 20.0,
                    -display.height * 0.5 + 20.0,
                    0.1,
                ],
            );
        }
