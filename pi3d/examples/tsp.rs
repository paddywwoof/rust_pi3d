...
use std::{cmp::Ordering, collections::HashMap, fs::File, io::prelude::*};
...
fn main() {
    ...
    let flatsh = pi3d::shader::Program::from_res("mat_flat").unwrap();
    let pointsh = pi3d::shader::Program::from_res("mat_pointsprite").unwrap();
    ...
    match File::open("cities/cities80.txt") {
        // NB needs to be in step with N_CITIES define above
        Ok(mut f) => {
            ...
            match f.read_to_string(&mut buffer) {
                Ok(_n) => {
                    ...
                    for i in b {
                        let pos: Vec<f32> =
                            i.split(",").filter_map(|s| s.trim().parse().ok()).collect();
                        if pos.len() >= 2 {
        ...
        Err(e) => {
            ...
            for _i in 0..N_CITIES {
                cities.push(Point::new(
                    rng.gen_range(-0.45 * W, 0.45 * W),
                    rng.gen_range(-0.45 * H, 0.45 * H),
                ));
            }
            for c in cities.iter() {
                println!("Point::new({:.1}, {:.1}),", c.x, c.y);
            }
        }
    }
    ...
    for i in 0..N_CITIES {
        let blk = labels.add_text_block(
            &font,
            &[cities[i].x - 5.0, cities[i].y + 5.0, 0.0],
            3,
            &format!("{}", i),
        );
    ...
    let score_blk = score.add_text_block(
        &font,
        &[-W * 0.5 + 10.0, H * 0.5 - 50.0, 0.0],
        15,
        "----------",
    );
    ...
    for _i in 0..POP_N {
        population.push(Organism::new(cities.len(), &mut rng));
    }
    ...
    while display.loop_running() {
        // escape key and exit included by default
        route.draw();
        points.draw();
        labels.draw();
        score.draw();
        if recalc {
            ...
            for i in 3..15 {
                // mutate some of the good results shuffling 2 to 5 genes
...
impl Organism {
    fn new(n_cities: usize, rng: &mut ThreadRng) -> Organism {
        let mut orgm = Organism {
            genes: (0..n_cities).collect(),
            length: -1.0f32,
        };
        orgm.shuffle(rng);
        orgm
    ...
    fn calc_length(
        &mut self,
        cities: &Vec<Point>,
        dist_table: &mut HashMap<(usize, usize), f32>,
    ) -> f32 {
        if self.length > 0.0f32 {
            ...
            for i in 0..self.genes.len() {
                ...
                let cn = if i == self.genes.len() - 1 {
                    // wrap back to start
                    self.genes[0] // cn is city next
                } else {
                    self.genes[i + 1]
                };
                let d_key = if cn > c { (c, cn) } else { (cn, c) }; // reduce size of lookup table as a->b same as b->a
