...
macro_rules! make_shape {
    ($s:ident) => { // recursively calls macro with empty braces for additional attributes
        make_shape!($s, );
    };
    ($s:ident, // identifier to be used for this type of shape (i.e. Cube, Elevationmap etc)
     $($att:ident : $typ:ty = $val:expr) , *) => { /* repeating pattern of optional
     additional attributes in the form of identifier : type = default value i.e.
...
make_shape!(Shape); // use the generic macro to create a struct definition 'Shape'
