...
pub fn create(cam: Rc<RefCell<::camera::CameraInternals>>,
              radius: f32, thickness: f32, height: f32, sides: usize, use_lathe: bool
              ) -> ::shape::Shape {
    let t = thickness * 0.5;
    if use_lathe {
        ...
        let normdirs: [[f32; 2]; 4] = [[0.0, 1.0],  //up
                                       [1.0, 0.0],  //out
                                       [0.0, -1.0], //down
                                       [-1.0, 0.0]];//in
        for i in 0..=sides {
            for (j, (xz, y)) in [(inr, ht),  (otr, ht), // up
                                 (otr, ht),  (otr, -ht),// out
                                 (otr, -ht), (inr, -ht),// down
                                 (inr, -ht), (inr, ht)].iter().enumerate() { // in
                let s = (i as f32 * step).sin();
                let c = (i as f32 * step).cos();
                verts.extend_from_slice(&[xz * s, *y, xz * c]);
                let k: usize = j / 2;
                norms.extend_from_slice(&[normdirs[k][0] * s,
                                          normdirs[k][1],
                                          normdirs[k][0] * c]);
                if k == 0 || k == 2 { // top or bottom
                    uvs.extend_from_slice(&[0.5 * (1.0 + verts[j * 3] / otr),
                                            0.5 * (1.0 + verts[j * 3 + 2] / otr)]);
                } else {
                    uvs.extend_from_slice(&[i as f32 / sides as f32,
                                            0.5 * (1.0 + y / ht)]);
                }
            }
            if i < sides {
                for (a, b, c) in [(0, 1, 8),  (1, 9, 8),
                                  (2, 3, 10), (3, 11, 10),
                                  (4, 5, 12), (12, 5, 13),
                                  (6, 7, 14), (7, 15, 14)].iter() {
