...
pub fn add_buffers(merge_to: &mut ::shape::Shape, new_bufs: Vec<&::buffer::Buffer>,
    ...
    for i in 0..new_bufs.len() {
        ...
        while num[i] >= merge_to.buf.len() {
            ...
            verts.push(merge_to.buf[0].array_buffer.slice(s![0..0,0..3]).to_owned());
            norms.push(merge_to.buf[0].array_buffer.slice(s![0..0,3..6]).to_owned());
            tex_coords.push(merge_to.buf[0].array_buffer.slice(s![0..0,6..8]).to_owned());
            faces.push(merge_to.buf[0].element_array_buffer.slice(s![0..0,..]).to_owned());
            new_buf_ix.push(0);
        }
