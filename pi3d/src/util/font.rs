...
pub fn create(file_name: &str, glyphs: &str, add_glyphs: &str, size: f32) -> TextureFont {
    ...
    let font = Font::from_bytes(contents).expect("Error constructing Font");
    ...
    for (i, c) in glyph_list.chars().enumerate() {
        ...
        if let Some(g_b_box) = g.exact_bounding_box() {
            // only do this if there's a glyph
            cpoint.x =
                ((i + 1) % grid_n) as f32 * size + (size - g_b_box.max.x - g_b_box.min.x) * 0.5;
            cpoint.y = ((i + 1) / grid_n + 1) as f32 * size; //TODO magic numbers
            let g = g.positioned(cpoint);
            if let Some(p_b_box) = g.pixel_bounding_box() {
                //TODO error catching
                g.draw(|x, y, v| {
